...
(function() {
  ...
  const START_ALERT_KEY = 'mp_start_alert_triggered';
  ...
  function loadStartAlertState() {
    try {
      const data = localStorage.getItem(START_ALERT_KEY);
      if (data) {
  ...
  function saveStartAlertState() {
    try {
      localStorage.setItem(START_ALERT_KEY, JSON.stringify(startAlertTriggered));
    } catch (e) {}
  }
  ...
  function saveRecordedEvents(events) {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(Array.from(events)));
    } catch (e) {}
  }
  ...
  function sendTeamNamesToBg(teams) {
    if (teams.home !== '主队' || teams.away !== '客队') {
      safeSendMessage({
        type: 'SET_TEAM_NAMES',
        data: { home: teams.home, away: teams.away }
      });
    }
  }
